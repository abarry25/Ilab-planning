...
function renderMilestoneRow() {
  ...
  spacer.className = "corner-spacer milestone-spacer";
...
function renderWeekHeader() {
  ...
  corner.className = "corner-spacer week-spacer";
  corner.textContent = term.label + " · " + (viewMode === "day" ? "days" : "weeks");
...
function taskRowMarkup(t) {
  ...
  row.className = "task-row" + (STATE.selected[t.id] ? " row-selected" : "");
  ...
  check.checked = !!STATE.selected[t.id];
...
function renderBoardRowInPlace(oldRow, t) {
  const fresh = taskRowMarkup(t);
  oldRow.replaceWith(fresh);
}

function updateRowCount(row, st) {
  const activeN = st.active.filter(Boolean).length;
  row.querySelector(".row-count").textContent = activeN > 0 ? activeN + "d" : "";
}
...
function renderBoard() {
  ...
  GROUPS.forEach(g => {
    ...
    if (STATE.collapsed[g.id]) section.classList.add("collapsed");

    const header = document.createElement("div");
    header.className = "group-header";
    header.style.setProperty("--gcolor", g.color);
    header.innerHTML = `
      <span class="group-tab" style="background:${g.color}"></span>
      <button type="button" class="collapse-btn" aria-label="Toggle group">▾</button>
    ...
    header.querySelector(".collapse-btn").addEventListener("click", () => {
      ...
      STATE.collapsed[g.id] = section.classList.contains("collapsed");
      queueSave();
    });
    ...
    blocks.forEach(block => {
      ...
      block.tasks.forEach(t => body.appendChild(taskRowMarkup(t)));
    ...
    if (!blocks.length) {
      ...
      addRow.addEventListener("click", () => addTask(g.id, null));
      body.appendChild(addRow);
    }
